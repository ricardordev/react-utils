...
export function SearchRepo() {
  ...
  const fetchGitHubAPI = useCallback(async (queryTerm: string, source: 'Manual Button' | 'Debounce Effect') => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Error searching for GitHub data.';
      setErrorApi(message);
      addLog(`[${source}] Search failed with term: "${queryTerm}". Msg: ${message}`, 'error');
    } finally {
      setLoading(false);
    }
  }, [addLog]);
